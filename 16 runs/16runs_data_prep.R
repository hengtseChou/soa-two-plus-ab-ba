...
write.csv(indep_cols, "16runs_indep.csv", row.names=F)
...
generator <- t(full(2, 4)[-1, 4:1])
write.csv(generator, "16runs_generator.csv", row.names=F)
...
for (i in 1:ncol(C2)) {
  ...
  tmp <- which(2 ^ (0:3) %*% generator == sum(s2 * 2 ^ (0:3)))
...
write.csv(unique_lines, "16runs_lines.csv", row.names=F)
...
write.csv(good_A, "16runs_good_A.csv", row.names = FALSE)
