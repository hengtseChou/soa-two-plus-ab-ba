...
orig <- catalogue %>% 
  filter(num_of_columns >= 10, num_of_columns <= 15) %>%
  nrow()
comp <- catalogue %>% 
  filter(num_of_columns >= 7, num_of_columns <= 15) %>%
  nrow()
cat(paste0("Num of regular designs: ", orig + comp, "\n"))

indep_cols <- read.csv("32runs_indep.csv") |>
  as.matrix()
generator <- read.csv("32runs_generator.csv") |>
  as.matrix()
lines <- read.csv("32runs_lines.csv") |>
  as.matrix()

# load in design A of SOA 2+
# wlp in good_A starts at length = 3
good_A <- read.csv("32runs_good_A.csv")
good_A <- good_A %>%
...
write.csv(result, "32runs_alpha_beta.csv", row.names = F)
...
write.csv(filtered, "32runs_beta_alpha.csv", row.names = F)
